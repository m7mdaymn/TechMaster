...
import { DashboardService, Instructor } from '@core/services/dashboard.service';
      ...
      <div class="editor-header">
        <div class="header-left">
          ...
          <div class="header-info">
            ...
            <span class="subtitle">Admin Course Editor</span>
          </div>
        </div>
              ...
              <div class="form-group">
                ...
                <select [(ngModel)]="course.instructorId">
                  ...
                  @for (instructor of instructors(); track instructor.id) {
                    <option [ngValue]="instructor.id">{{ instructor.fullName }}</option>
                  }
                </select>
              </div>
...
export class AdminCreateCourseComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (courseId) { this.isEditMode.set(true); this.loadCourse(courseId); }
  }

  loadInstructors() {
    this.dashboardService.getInstructors().subscribe({
      next: (instructors) => this.instructors.set(instructors),
      error: () => this.toastr.error('Failed to load instructors')
    });
  }
  ...
  loadSiteSettings() {
    this.settingsService.getAllSettings().subscribe({
      next: (settings) => {
        const currencySetting = settings?.find(s => s.key === 'site.currency' || s.key === 'currency');
        if (currencySetting) {
          this.siteCurrency.set(currencySetting.value || 'EGP');
        }
      }
    });
  }
          ...
          if (c.modules && c.modules.length > 0) {
            this.modules.set(c.modules.map((m: any, mi: number) => ({
              ...
              sessions: (m.sessions || []).map((s: any, si: number) => ({
                ...
                videoUrl: s.videoUrl, pdfUrl: s.pdfUrl, content: s.content, externalLink: s.externalLink,
  ...
  isStepCompleted(stepId: string): boolean {
    if (stepId === 'basic') return !!(this.course.nameEn && this.course.instructorId && this.course.categoryId);
  ...
  isValid(): boolean { return !!(this.course.nameEn && this.course.instructorId && this.course.categoryId); }
  ...
  saveCourse(status: string) {
    ...
    if (!this.course.instructorId) { 
      this.toastr.error('Please select an instructor'); 
      this.activeStep.set('basic');
      return; 
    }
    ...
    const courseId = this.route.snapshot.queryParamMap.get('id');
    const request$ = courseId ? this.courseService.updateCourse(courseId, courseData as any) : this.courseService.createCourse(courseData as any);
    request$.subscribe({
      next: (response) => {
